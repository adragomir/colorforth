...
fr_:
	DUP_
	mov eax, (frame-start)/4
	ret

fo_:
	DUP_
	mov eax, (fore-start)/4
	ret
...
keyboard:
	...
	call color
	mov dword [rm], hc*iw
	mov dword [lm], hp-9*iw
	mov dword [xy], (hp-9*iw)*0x10000+vp-4*ih
	call eight
	call eight
	call eight
	call cr

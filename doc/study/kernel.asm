...
; defines for video mode -- one of:
%if VIDEO = 117
 %include '117h.inc'	; VESA mode 4117h (1024x768 16-bit color)
%elif VIDEO = 118
 %include '118h.inc'	; VESA mode 4118h (1024x768 32-bit color)
%else 
 %error please define VIDEO to be 117 or 118.
%endif
...
ch_w	equ ch_pad+iw+ch_pad			; 22
ch_h	equ ch_pad+ih+ch_pad			; 30
; horizontal/vertical characters (screen size)
hc	equ hp/ch_w				; 1024 / 22 = 46 (remainder 12)
vc	equ vp/ch_h				; 768 / 30 = 25 (remainder 18)
...
rsize	equ 768*4   ; size of return stacks (3 blocks)
dsize	equ 1536*4  ; size of data stacks (6 blocks)
...
debug:
	mov dword [xy], ch_pad*0x10000+(vc-2)*ch_h+ch_pad
	DUP_
	mov eax, [main]
	push dword [eax]
	call dot
...
down:
	...
	div ecx
	mov eax, edx
	add edx, ch_pad*0x10000 + 0x8000-ch_h+ch_pad
	mov [xy], edx
zero:	test eax, eax
	mov eax, 0
	jnz .9
...
keyboard:
	...
	mov dword [lm], ch_pad
	mov word [xy+2], ch_pad
	call stack
	; display input history just to the left of keyboard display
	mov word [xy+2], hp+ch_pad - (histsz+kbw)*ch_w
	lea edi, [history-4]
	mov ecx, histsz
	jmp nchars
...
xkb:	dd hp - kbw*ch_w + ch_pad 
ykb:	dd vp - kbh*ch_h + ch_pad

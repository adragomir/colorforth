...
start1:
    ...
    call setgraphics
    lidt [idt] ;# enable division-exception recovery
...
act: ;# set currently active task
    ...
    mov  main, eax ;# 'main' return stack pointer in 'main' slot
    drop ;# what was 'dup'd before now into eax
    ret ;# to previous caller, since we already popped 'our' return address

show0: call show
    ret
show: pop screen ;# return address into screen; 'ret' if from show0
    dup_
    xor  eax, eax
...
abort1:
    ...
    mov  dword ptr spaces+3*4, offset forthd ;# adefine
...
forthd:
    ...
    inc  dword ptr forths                      ; # make it one more, point to the slot for the next definition
...
0:  mov  edx, [loadaddr-4+edi*4]                          ; # load the packed word from source block
    ...
    mov  edx, h                                ; # 'here' pointer for new compiled code
...
cdrop:                          ; # compile DROP
    mov  edx, h      ; # get HERE address
    mov  list, edx   ; # save in 'list' for possible tail optimization
    mov  byte ptr [edx], 0x0ad  ; # compile 'lodsd'=DROP
    inc  dword ptr h ; # move HERE forward accordingly
    ret

qdup:                           ; # compile DUP only if needed (tail optimization)
    mov  edx, h      ; # point to...
    dec  edx                    ; # ...last byte compiled
    cmp  list, edx   ; # was this address saved in 'list'?
    jnz  cdup                   ; # if not, compile the DUP
    cmp  byte ptr [edx], 0xad   ; # is it a LODS operator (DROP)?
    jnz  cdup                   ; # DUP if not
    mov  h, edx      ; # adjust HERE pointer
    ret
...
cdup:                                ; # compile DUP
    ...
    add  dword ptr h, 5   ; # point past compiled code
    ret
...
literal: ;# compile the literal (number) in EAX
    call qdup
    mov  edx, list ;# get what was last saved in 'list'
    mov  list + 4, edx ;# save that in next slot...
    mov  edx, h ;# now get HERE pointer
    mov  list, edx ;# and store that in 'list'
    mov  byte ptr [edx], 0x0b8 ;# compile 'mov eax, N'
    mov  [edx+1], eax  ;# store the N
    add  dword ptr h, 5 ;# adjust HERE pointer
    ret
...
call_:
    mov  edx, h ;# get 'here' pointer, where new compiled code goes
...
0:  mov  byte ptr [5+edx], 0x0c3 ;# compile a 'ret' at end
    inc  dword ptr h  ;# make 'here' pointer reflect that
    ret
...
debug: ;# show current machine state
    ...
    push [xy]
    mov  dword ptr [xy], 3*0x10000+(vc-2)*ih+3
    dup_
    mov  eax, god
    push [eax]
    call dot
...
switch: ;# refresh screen, then switch tasks
    ...
    push edi
    mov  edi, displ ;# linear memory of video controller
...
graph0:
	long nul0, nul0, nul0, alph0
    .byte  0 ,  0 ,  5 , 0             ; #     a
graph1:
	long word0, x, lj, alph
    .byte 025, 045,  5 , 0             ; # x . a
alpha0:
	long nul0, nul0, number, star0
    .byte  0 , 041, 055, 0             ; #   9 *
alpha1:
	long word0, x, lj, graph
    .byte 025, 045, 055, 0             ; # x . *
numb0:
	long nul0, minus, alphn, octal
    .byte 043,  5 , 016, 0             ; # - a f
numb1:
	long number0, xn, endn, number0
    .byte 025, 045,  0 , 0             ; # x .
...
accept:
acceptn:
	mov dword ptr shift, offset alpha0
    lea  edi, alpha - 4
accept1:
	mov board, edi
0:  call key
...
octal:
	xor dword ptr current, (offset decimal-offset start) ^ (offset hex-offset start)
...
qdot: ;# display number as decimal or hexadecimal (hex)
    cmp dword ptr base, 10 ;# is current base decimal?
...
type0: ;# display continuation of previous word
    ...
    jnz  type1
    dec  edi
    mov  lcad, edi  ;# save address of leftmost part of word
    call space
    call qring
...
refresh: call show
    ...
    call text1
    dup_ ;# counter
    mov  eax, lcad ;# get address of leftmost part of word
    mov  cad, eax ;# for curs beyond .end
    xor  eax, eax
    mov  edi, blk ;# get current block, which is being edited
...
0:  mov  edx, [loadaddr+edi*4]
    ...
    mov dword ptr bas, offset dot ;# display as hex
0:  and  edx, tagbits
...
;# action for number
actn: mov  keyc, eax  ;# store key color
    pop  eax
    drop
    jmp  accept
;# after 'm' pressed, change color and prepare to store variable name
actv:
    mov  byte ptr action, 12 ;# variable
...
mblk: ;# minus one block (-2 since odd blocks are shadow blocks)
    cmp dword ptr blk, 20 ;# minus one block unless below 20
...
e:  dup_
    mov  eax, blk
    mov  dword ptr anumber+loadaddr, offset format
    mov  byte ptr alpha0+loadaddr+4*4, 045 ;# .
    mov  dword ptr alpha0+loadaddr+4, offset e0
    call refresh
...
eout: pop  eax
    ...
    mov  dword ptr alpha0+loadaddr+4, offset nul0
...
format:
    test byte ptr action, 0xa ;# ignore 3 and 9
    jz   0f
    drop
    ret
0:  mov  edx, eax
...
enstack: dup_
    mov  eax, cad  ;# current cursor address
    ...
    push esi  ;# ;# save data stack pointer
    mov  esi, cad ;# point ESI to where cursor is
    lea  esi, [loadaddr+esi*4-4] ;# make it an absolute address
    mov  edi, trash ;# get pointer into edit buffer
0:  std  ;# move backwards with ESI...
    ...
    mov  trash, edi ;# store the updated pointer
    pop  esi ;# restore data stack pointer

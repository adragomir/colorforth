...
show0:
    ...
    ret
show:
    pop  screen   ; pop return address into screen, 'ret' if from show0
...
ex1:
    dec dword ptr words ; from keyboard
    jz   0f
    drop
    jmp  ex1
0:  call find
...
cdup: ; compile a DUP instruction
    ...
    add dword ptr  h, 5 ; adjust HERE accordingly, point past compiled code
    ret
...
cshort:  ; compile short (27 bits or less) signed number
    call [lit]
    mov  eax, [-4+edi*4]; get number from preparsed source
...
semi: ; end definition
    ...
    cmp  list, edx
    jnz  0f
    cmp  byte ptr [edx], 0x0e8 ; was it a 'call' instruction?; e8=call, e9=jmp
...
copy:
	cmp  eax, 12 ; can't overwrite machine-code blocks...
...
debug: ; show current machine state
    ...
    mov dword ptr  xy,  offset (3*0x10000+(vc-2)*ih+3)
    dup_
    mov  eax, god
    push [eax]
    call dot
...
xy_: dup_ ; return xy address in words to high-level application
...
; keystroke processing under Bochs.
    dup_             ; save copy of return stack pointer(?); save EAX, will be restored by caller using 'drop'
...
0:  and  edx, tagbits ; get typetag bits as index into display vector
...
pcad: .long 0 ; "previous"/page cursor address
lcad: .long 0 ; "last" / left cursor address of multipart word)
trash: .long buffer*4
.endif
...
format: 
    test byte ptr action, 012 ; ignore 3 and 9
    jz   0f
    drop
    ret
0:  mov  edx, eax
    and  edx, 0x0fc000000 ; check if we have room for tagbits, hexbit, sign bit

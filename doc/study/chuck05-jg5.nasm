...
.0: call find ; Look up word in FORTH wordlist.
...
.0: call find          ; Otherwise, find it in FORTH wordlist.
...
; Compiles code to return to caller (either JMP or RET).
  ...
  ret
.0: mov byte [edx + 5], op_ret        ; convert call to ret
  inc dword [H]
  ret
...
switch_:
  ...
  push edi
  mov edi, [displ]
  mov ecx, screen_width * screen_height * screen_depth / 4
  rep movsd
  pop edi
  pop esi
  ret
...
line:
  call pen_addr
  mov ecx, [esi]
  shl ecx, bpplog2  ; ecx = ecx * screen_depth
  sub edi, ecx
  mov ecx, eax
  mov eax, [fore]
  REP_STOPIX
...
.1: mov eax, [xc]
  ...
  sub edx, [esi]
  shl edx, bpplog2  ; edx = edx * screen_depth
  mov eax, [fore]
.2: push ecx
  mov ecx, [esi]
  REP_STOPIX
...
.0: cmp edx, [esi]        ; (cmp modified 2nd arg, 1st arg)
...
key:
  DUP_
  xor eax, eax
.0: call dopause ; call dopause to let the other task run
...
.1: call odig    ; Get next hexadecimal digit.
.2: call edig    ; Output digit.
  NEXT .1      ; Continue until there are no more digits.
  call space
  DROP
  ret
.3: inc ecx      ; (Go from handling leading zeroes

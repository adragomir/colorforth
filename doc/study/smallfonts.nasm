...
.em:	push ecx
	...
	pop esi
bl_:	DROP
space:	add dword [xy], iw*0x10000/2
	ret
...
keyboard:
	...
	call color
	mov dword [rm], hc*iw
	mov dword [lm], hp-10*iw/2+3
	mov dword [xy], (hp-10*iw/2+3)*0x10000+vp-4*ih+3
	call eight
	call eight
	call eight
	call cr
	add dword [xy], 4*iw/2*0x10000
	mov edi, [shift]
	add edi, byte 4*4-4
	mov ecx, 3
	call four1
...
ring:
	mov dword [cad], edi
	sub dword [xy], iw/2*0x10000		; bksp
	DUP_
	mov eax, 0x0e04000
	call color
	mov eax, 60q
...
type0:
	sub dword [xy], iw/2*0x10000		; call bspcr
	test dword [-4+edi*4], -20q
	jnz type_
	  dec edi
	  mov dword [lcad], edi
